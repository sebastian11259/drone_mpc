...
from drone_sim.gym_pybullet_drones.utils.enums import DroneModel, Physics, ImageType

from drone_sim.gym_pybullet_drones.envs.BaseAviary import BaseAviary
...
class MPCAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        init_pos, init_quat = self.track.getTrackStart()

        initial_xyzs = init_pos.reshape(self.NUM_DRONES,3)
        initial_rpys = np.array(p.getEulerFromQuaternion(init_quat)).reshape(self.NUM_DRONES,3)

        self.curr_segment_idx = 0
        self.current_segment = self.track.segments[0]
    ...
    def step(self,
        ...
        if (self._gateScored() and self.current_segment.segmentFinished(obs[0:3])):
